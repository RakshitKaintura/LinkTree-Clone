...
export default async function Page({ params }) {
    ...
    return (
        <Link key={index} href={item.link}>
            {/* Added 'text-purple-900' below to make the text visible */}
            <div className="bg-purple-100 py-4 shadow-lg px-2 min-w-96 flex justify-center rounded-md my-3 text-purple-900 font-medium">
                {item.linktext}
            </div>
        </Link>
    )
